...
const handleIndicatorClick = (type: IndicatorType) => {
  const filter = encodeURIComponent(JSON.stringify(dimensionFilter.value))
  uni.setStorageSync('reports:dimensionFilter', dimensionFilter.value)
  uni.navigateTo({
    url: `/pages/other/reports/indicator-detail?type=${type}&dimensionFilter=${filter}`
  })
}
